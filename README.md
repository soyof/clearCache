...
![版本](https://img.shields.io/badge/版本-1.7.1-blue.svg)
...
- **🐛 修复版本号显示问题** - 解决版本号显示"加载中..."的问题，移除内联脚本避免 CSP 违规
- **⚡ 性能优化** - 优化清理操作响应速度，立即显示反馈，主要操作并行执行
- **🧹 代码清理** - 移除冗余的版本号加载函数，简化代码结构
...
- **🐛 修复右键菜单兼容性** - 解决内网地址和特定页面上右键菜单不显示的问题
- **🚀 优化 macOS 性能** - 消除 macOS 系统上"清空缓存并硬性重新加载"的 1-3 秒延迟
- **✨ 智能菜单显示** - 自动排除扩展程序页面和浏览器内部页面，界面更清晰
- **🎯 增强错误处理** - 在受保护页面上提供友好的错误提示

#### v1.6.0 - 零延迟响应

- **重载功能优化** - 彻底解决重新加载延迟问题
  - 从 500-1000ms 优化到 <5ms，性能提升 99%+
  - 移除所有中间层调用，直接调用 Chrome API
  - 采用强制重排机制，确保 UI 立即更新
  - 按钮状态和提示消息瞬间可见
- **智能异步处理** - 缓存清理不阻塞页面重载
  - 先触发页面重载，再后台清理缓存
