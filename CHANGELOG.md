...
- **清理时间范围选择** - 新增清理时间范围选择功能，支持按时间段清理浏览器数据

  - 支持选择最近 1 小时、24 小时、7 天、4 周或全部时间

- **全局存储分析导出功能** - 新增全局存储分析页面的数据导出功能

  - 支持导出数据（JSON/CSV 格式）
  - 支持导出表格（Excel 格式）
  - 支持导出图表（PDF 格式）
  - 方便用户备份分析数据或进行进一步的数据处理

- **国际化文件同步优化** - 清理并同步所有语言包的国际化键值
  - 删除所有语言文件中多余的 `totalSize` 键
  - 补全缺失的国际化键值，确保所有语言文件完全同步

## v1.7.7 (2025-12-10)

### ✨ 新功能

- **全局存储分析页面重大升级** - 全面重构分析页面，新增丰富的多维度数据可视化分析功能
  - **Tab 页签导航** - 新增"表格"和"图表"两个标签页，方便在不同视图间切换
  - **多维度图表分析** - 新增 11 个分析视角，包含 30+ 个图表：
    - 存储类型视角：按存储类型分布（大小/条目数）
    - 域名视角：Top 域名（按估算大小/条目数/Cookies 大小）
    - 域名与类型视角：Top 域名存储类型堆叠（大小/条目数）
    - 存储质量与风险：存储条目大小分布、Cookies 安全属性、过期分布、7 天内即将过期的 Cookies、存储质量/风险列表
    - 效率与密度视角：各存储类型平均条目大小、域名存储密度分布、域名大小与条目数散点图
    - 集中度视角：存储集中度分析（帕累托图）、Top 域名占比分析
    - 分布视角：域名大小分布直方图、域名条目数分布直方图、存储类型使用率
    - 对比视角：存储类型效率对比、域名存储类型偏好
    - 安全与风险视角：不安全 Cookies 域名 Top、过期 Cookies 域名分布、Cookies 安全属性组合分析
    - 细分视角：按顶级域名（TLD）分组统计、按域名长度分组统计、按存储类型组合分组
    - 统计摘要视角：存储统计摘要、存储类型大小分布箱线图
  - **图表导航功能** - 新增右下角浮动导航按钮，支持快速跳转到各个分析章节
  - **滚动到顶部按钮** - 新增滚动到顶部快捷按钮，提升长页面浏览体验

### 🎨 UI/UX 优化

- **页面布局优化** - 优化分析页面布局，移除页面级滚动条，改为内容区域内部滚动
  - 表格页面：页面不滚动，表格内容区域内部滚动
  - 图表页面：图表区域可滚动，支持查看所有图表
- **图表布局优化** - 图表采用网格布局，一行显示两个图表，保持整齐美观
- **Tab 切换动画** - 添加平滑的淡入淡出动画效果，提升交互体验
- **滚动条样式统一** - 统一所有滚动条样式，使用渐变背景和毛玻璃效果，保持视觉一致性
- **锚点导航优化** - 优化锚点跳转逻辑，确保准确跳转到目标位置，无多余间距

### 🐛 问题修复

- **修复图表文本模糊问题** - 优化 Canvas 渲染，确保图表文本清晰锐利
- **修复图表显示空白问题** - 优化图表渲染时机，确保在 Tab 切换后正确显示
- **修复数字精度问题** - 修复 TLD 分布图表中浮点数精度显示问题，确保显示整数
- **修复图表布局问题** - 修复图表换行问题，确保始终一行显示两个图表
- **修复导航按钮定位问题** - 修复浮动导航按钮位置不正确的问题，确保固定在右下角
- **修复搜索框显示问题** - 切换到图表页面时自动隐藏搜索框，切换到表格页面时显示

### 🔧 技术改进

- **代码模块化重构** - 将大型 `analysis.js` 文件拆分为多个专注的模块：
  - `analysisUtils.js` - 工具函数（格式化、空状态等）
  - `analysisDataCollector.js` - 数据收集逻辑
  - `analysisDataCleaner.js` - 数据清理逻辑
  - `analysisTableRenderer.js` - 表格渲染逻辑
  - `analysisChartRenderer.js` - 图表渲染协调器
  - `chartCore.js` - 图表核心渲染函数
  - `chartConfig.js` - 图表通用配置
  - `chartTypes.js` - 存储类型视角图表
  - `chartDomains.js` - 域名视角图表
  - `chartStacks.js` - 域名与类型视角图表
  - `chartQuality.js` - 质量与风险视角图表
  - `chartEfficiency.js` - 效率与密度视角图表
  - `chartConcentration.js` - 集中度视角图表
  - `chartDistribution.js` - 分布视角图表
  - `chartComparison.js` - 对比视角图表
  - `chartSecurity.js` - 安全与风险视角图表
  - `chartSegmentation.js` - 细分视角图表
  - `chartStatistics.js` - 统计摘要视角图表
- **性能优化** - 优化首次加载和图表渲染性能
  - 实现图表分批渲染机制，使用 `requestAnimationFrame` 避免阻塞 UI
  - 延迟图表渲染，仅在切换到图表标签页时才渲染
  - 优化数据加载流程，使用 `requestIdleCallback` 提升响应速度
- **Chart.js 集成** - 全面采用 Chart.js 库进行图表渲染，提供更丰富的图表类型和更好的性能
- **国际化完善** - 为所有新增图表和分析维度添加完整的多语言支持

## v1.7.6 (2025-12-10)

### 🎨 分析页优化

- 新增“按存储类型分布”与“Top 域名展示”模块，置于表格下方，方便对比汇总数据与明细。

## v1.7.5 (2025-12-10)

### 🛠 构建与发行

- 调整 GitHub Actions 打包标签：自动标签命名改为 `缓存清理助手-<版本号>`，如同名标签已存在则跳过创建。

## v1.7.4 (2025-12-08)

### ✨ 新功能

- **扩展级域名过滤功能** - 新增黑白名单域名过滤机制，支持为清理、右键菜单、存储扫描设置域名过滤规则

  - 支持三种过滤模式：禁用、白名单、黑名单
  - 白名单模式：仅允许列表中的域名执行清理操作；黑名单模式：阻止列表中的域名执行清理操作
  - 支持通配符域名（如 `*.example.com`）和子域名匹配规则
  - 集成到清理操作、右键菜单和存储扫描功能中

- **域名批量添加弹窗** - 弹窗式添加界面，支持连续添加多个域名，实时显示已添加列表，添加成功后统一保存

### 🎨 UI/UX 优化

- **高级设置页面布局优化** - 清理选项和通知设置改为两列网格布局，域名过滤设置保持单列显示，统一 padding 和间距，使界面更紧凑

- **域名过滤操作优化** - 移除保存按钮改为即时保存，删除域名时添加二次确认对话框，优化弹窗和按钮样式尺寸

### 🐛 问题修复

- **修复 CSV 导出中文乱码问题** - 在 CSV 文件开头添加 UTF-8 BOM，确保中文正确显示
- **修复国际化显示问题** - 确保域名过滤相关文本在所有语言下正确显示

### 🔧 技术改进

- **代码模块化优化** - 将域名过滤功能拆分为独立模块（`utils/domainFilter.js`、`popup/domainFilterUI.js`、`utils/domainAddModal.js`），提升代码可维护性
- **国际化完善** - 为所有新增功能添加完整的多语言支持（8 种语言）

## v1.7.3 (2025-12-08)

### ✨ 新功能

- **新增存储数据导出功能** - 在存储详情弹窗中新增 JSON/CSV 导出功能，支持导出全部数据或仅导出筛选结果，方便数据备份与分析
- **完善全局存储分析页面** - 优化存储分析页面交互体验，新增域名搜索功能，支持按域名快速定位和清理存储数据

### 🐛 问题修复

- **修复语言切换后导出按钮文本不更新问题** - 优化国际化更新机制，确保导出相关按钮文本随语言切换实时更新
- **修复语言文件重复键问题** - 清理所有语言文件中的重复 `cacheAPI` 键定义，消除 JSON 解析错误

### 🎨 UI/UX 优化

- **优化存储详情导出区域布局** - 调整导出按钮与筛选选项的排列方式，改善视觉拥挤问题，提升操作体验
...
- 全局清理操作支持静默执行，避免频繁弹窗。
- 重建“全局存储分析”页面：按域名汇总 Local/Session/IndexedDB/Cache/Cookies，支持单域与全量清理。
